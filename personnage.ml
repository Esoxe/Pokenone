...
let movement l_texture =
  ...
  let bord_droite = !x<800-26 && (collision_checker (!x+2) !y l_texture) in
  let bord_gauche = !x>0 && (collision_checker (!x-2) !y l_texture) in
  let bord_haut =   !y>0 && (collision_checker !x (!y-2) l_texture) in
  let bord_bas =    !y<500-36 && (collision_checker !x (!y+2)l_texture)  in
